...
<?php
    ...
    if(isset($_POST['auth'])){
        ...
        if($_POST['username']=='admin' and $_POST['email']=='admin@mail.ru' and $_POST['password']=='adminpassword'){
            ...
            $connection->exec('LOCK TABLES author WRITE, redactor WRITE, corrector WRITE, reviewer WRITE');
            $username = $_POST['username'];
        $email = $_POST['email'];
        ...
        $existigs_rev = $connection->prepare("SELECT * FROM reviewer WHERE passwd=:password and username=:u");
        ...
        if($existigs_rev->rowCount()){
            ...
            setcookie('remember_e',$email,time()+360000,'/');
            setcookie('remember_p',$password,time()+360000,'/');
            setcookie('remember_w','rev',time()+360000,'/');
            header("Location:page_rev.php");
        }
        }
    }
