...
class Author{
    ...
    function upload_zero_version($title, $date){
        ...
        $pnam = $title.$version;
        move_uploaded_file($tname,'C:\wamp\www\redaction\versions'.'/'.$pnam.$version);
    ...
    function upload_to_server(){
        if (isset($_POST["submit"]) and isset($this->name)){
            //sleep(10);
            if($this->stater($_POST["title"])>=10){
                echo('<h3>Невозможно отправить новую версию статьи, которая уже есть в номере<h3>');
                return 0;
            }
            ...
            if(count($res_array)>0){
                if($this->if_it_belongs($_POST["title"])==0){
                    echo('<h3>вы не можете отправить новую версию не вашей статьи</h3>');
                    return 0;
                }
                ...
                if($this->if_name_is_uniq($title)==1){
                    echo('<h3>статья с таким названием уже существует<h3>');
                    return 0;
                }
    ...
    function show_last_version(){
        if (isset($this->name)){
            $getter = $this->con->prepare('SELECT name, max(version_number) as version_number FROM version INNER JOIN send_recive ON send_recive.id_ver = version.id WHERE send_recive.sends = 1 AND send_recive.id_a=:t GROUP BY name');
    ...
    function show_all_problems(){
        ...
        $getter = $this->con->prepare('SELECT problem_list.txt, version.name FROM problem_list INNER JOIN version ON version.id =  problem_list.ver_id WHERE problem_list.ver_id IN (SELECT id_ver as ver_id FROM send_recive WHERE id_a =:f AND sends=1)');
    ...
    function show_corrected_versions(){
        
        $this->con->exec('LOCK TABLES send_recive WRITE, version_corrector WRITE, version WRITE, author WRITE');
        $getter = $this->con->prepare('SELECT MAX(send_recive.id_ver) as id, version.name as name FROM send_recive INNER JOIN version_corrector ON version_corrector.id_ver = send_recive.id_ver INNER JOIN version ON version.id = version_corrector.id_ver WHERE sends = 0 AND id_a = :f GROUP BY version.name');
        $getter->bindValue('f',$this->get_author_by_name(),PDO::PARAM_INT);
        $result = $getter->execute();
        $res_array = $getter->fetchALL(PDO::FETCH_ASSOC);
        //echo('<table style="background-color: red">');
        
        //echo('<tr><th>Название</th><th>Cсылка на скачивание</th></tr>');
        for($i =0;$i<count($res_array);$i++){
            echo("<h3>".$res_array[$i]["name"]."</h3>");
            echo('<p><a href="download.php?path=corrected_articles/review_'.$res_array[$i]['name'].$res_array[$i]['id'].$this->count_sended_version($res_array[$i]["name"]).'">Download TEXT file</a></p>');
        }
        //echo('</table>');
        $this->con->exec('UNLOCK TABLES');
    }
    ...
    function show_reviews_for_author(){
        $this->con->exec('LOCK TABLES review WRITE, version WRITE, reviewer WRITE, send_recive WRITE, author WRITE');
        $getter = $this->con->prepare('SELECT version.name as name, reviewer.username as username, max(version.id) as id FROM version INNER JOIN send_recive ON send_recive.id_ver=version.id INNER JOIN review ON version.id=review.ver_id INNER JOIN reviewer ON review.rev_id = reviewer.id WHERE send_recive.id_a=:t GROUP BY version.name');
        $getter->bindValue('t',$this->get_author_by_name(),PDO::PARAM_INT);
        $res = $getter->execute();
        $res_array = $getter->fetchALL(PDO::FETCH_ASSOC);
        $this->con->exec('UNLOCK TABLES');
        for($i =0;$i<count($res_array);$i++){
            echo("<h3>".$res_array[$i]["name"]."</h3>");
            echo('<p><a href=download.php?path=review/review_'.$res_array[$i]["name"].$res_array[$i]["id"].'">Download TEXT file</a></p>');
        }
